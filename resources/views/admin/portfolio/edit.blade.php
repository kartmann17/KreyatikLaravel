    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="card-title">Modification: {{ $portfolioItem->title }}</h3>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $portfolioItem->title) }}" required>
                        </div>

                        <div class="form-group">
                            <label for="description">Description <span class="text-danger">*</span></label>
                            <textarea class="form-control" id="description" name="description" rows="4" required>{{ old('description', $portfolioItem->description) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="technology">Technologies utilisées</label>
                            <input type="text" class="form-control" id="technology" name="technology" value="{{ old('technology', $portfolioItem->technology) }}" placeholder="ex: HTML, CSS, JavaScript, Laravel, etc.">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="file">Fichier (Image/Vidéo)</label>
                            <div class="input-group">
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" id="file" name="file" accept=".jpg,.jpeg,.png,.gif,.mp4,.webm">
                                    <label class="custom-file-label" for="file">Choisir un nouveau fichier</label>
                                </div>
                            </div>
                            <small class="form-text text-muted">Formats acceptés: JPEG, PNG, GIF, MP4, WEBM. Taille max: 20MB. Laissez vide pour conserver le fichier existant.</small>
                        </div>

                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_visible" name="is_visible" value="1" {{ old('is_visible', $portfolioItem->is_visible) ? 'checked' : '' }}>
                        ...
                        <div class="form-group mt-4">
                            <h5>Fichier actuel</h5>
                            @if($portfolioItem->isImage())
                                <img src="{{ asset($portfolioItem->path) }}" alt="{{ $portfolioItem->title }}" class="img-fluid img-thumbnail" style="max-height: 200px;">
                            @else
                                <video src="{{ asset($portfolioItem->path) }}" controls class="img-fluid img-thumbnail" style="max-height: 200px;">
                        ...
                        <div class="form-group mt-3">
                            <div id="preview-container" class="text-center d-none">
                                <h5>Aperçu du nouveau fichier</h5>
...
<script>
    $(function() {
        ...
        $('#file').on('change', function() {
            ...
            $(this).next('.custom-file-label').html(fileName || 'Choisir un nouveau fichier');
        ...
        $('#type').on('change', function() {
            $('#file').val('');
            $('.custom-file-label').html('Choisir un nouveau fichier');
